...
<script type="text/javascript">
	$j("#assignUserCourseTemplateBean").submit(
			function() {
			/*	if(noOptions($j("#toSelectBox"))==0)
					return false;*/
				...
				$j.postJSON($j('#assignUserCourseTemplateBean').attr(
...
<h2 id="saveAssignedTempMsg"></h2>
<form:form method="post" modelAttribute="assignUserCourseTemplateBean"
	...
	<div id="multiselectcontainer">
		...
		<div class="from_container">
			<span class="title"> Unassigned Templates </span><br /> <select
				id="fromSelectBox" multiple="multiple">
				<c:forEach var="template" items="${ALLTEMPLATELIST}"
					varStatus="status">
		...
		<div class="to_container">
			<span class="title"> Assigned Templates</span> <br /> <select
				id="toSelectBox" multiple="multiple" name="templateList">
				<c:if test="${not empty STUDENTTEMPLATE}">
					<c:forEach var="template" items="${STUDENTTEMPLATE}">
						<option Value="${template.id}">${template.name}</option>
					</c:forEach>
...
function doaction(data) {
	...
	if (message != "Saved") {
		...
		jAlert(" Student courseTemplate successfully "+message, 'Saved !');
	}
}
