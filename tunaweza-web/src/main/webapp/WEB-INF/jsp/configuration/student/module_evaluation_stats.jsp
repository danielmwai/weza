		...
		<c:otherwise>
			...
			<c:forEach var="evaluation" items="${EVALUATIONS}">
				<tr>
					...
					<td>
					<input type='button' value="retake" onclick="JJTEACH.moduleEvaluation('/studenttest/module/start','${evaluation.moduleId}');">
					</td>
				</tr>
			</c:forEach>
					...
					<div class="pages" id="pageLink" align="center"><c:if
						...
						<c:if test="${PAGE_NUMBER > 1}">
							<button class="pagenumb"
								onclick="JJTEACH.ajaxListModuleEvalStats('/user/module_evaluation_stats',${PAGE_NUMBER-1})"><c:out
								value="<< Previous" /></button>
						</c:if>
						<c:if test="${PAGE_NUMBER >2}">
							<button class="pagenumb"
								onclick="JJTEACH.ajaxListModuleEvalStats('/user/module_evaluation_stats',${PAGE_NUMBER-2})"><c:out
			...
			<tr><td colspan=5>Download|<a href="${PREFIX}/report/student/evaluation.htm?reportType=pdf">PDF<img src="${PREFIX}/images/icons/docs/pdf.png"/></a>|<a href="${PREFIX}/report/student/evaluation.htm?reportType=xls">EXCEL<img src="${PREFIX}/images/icons/docs/xls.png"/></a>|<a href="${PREFIX}/report/student/evaluation.htm?reportType=csv">CSV<img src="${PREFIX}/images/icons/docs/csv.png"/></a></td></tr>
		</c:otherwise>
	</c:choose>
</table>